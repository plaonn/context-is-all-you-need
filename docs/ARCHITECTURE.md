...
- `src/core/renderer.ts` owns escaped presentation-only HTML; the board computes one bounded unified Context graph plane with compact horizontal project coordinate lanes, a single global NOW rule, one continuous graph per project, Objective regions spanning all member nodes, and inline SVG branch/merge paths whose inputs are only explicit lineage edges. The accessible explicit edge list is a secondary disclosure. Lanes surface attention beside affected nodes and expanded lanes show bounded resume-critical fields; links point back to canonical Todoist.
