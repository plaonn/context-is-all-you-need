...
- The acceptance ledger records clean unpacked-extension acceptance through token exchange, authenticated task reads, and UI render without the DevTools `window.fetch` wrapper. Functional standalone cutover is ready; non-blocking UX follow-up remains outside this decision/task.
