...
Todoist OAuth uses a public client ID metadata URL, `data:read`, `chrome.identity.launchWebAuthFlow`, and PKCE. Tokens are kept in `chrome.storage.session`; disconnect removes local session state and does not attempt provider revocation. The Todoist data adapter issues GET requests only. The token endpoint POST is an OAuth exchange, not task/provider data mutation.
