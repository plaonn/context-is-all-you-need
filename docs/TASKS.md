...
| R3 public privacy and least privilege | `auth.ts`, `extension/config.ts`, local/session storage, escaped renderer | auth + first-run tests, privacy scan | Canonical OAuth registration/consent, PKCE token exchange, and provider scope readback |
...
The repository contribution is `implemented` after local validation, but standalone acceptance remains `not-ready` until direct canonical OAuth token exchange and first GET evidence exists. Provider/browser evidence is separate from tests and commits; no Todoist or production mutation is implied by this mapping.
