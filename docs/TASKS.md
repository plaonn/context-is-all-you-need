...
| R2 Context matrix, v1 semantics, bounded cache, Objectives, and material attention | `extension/config.ts`, `metadata.ts`, `projection.ts`, `pagination.ts`, `api.ts`, `cache.ts`, `renderer.ts`, `extension/styles.css` | config/projection/pagination/API/cache/renderer/layout tests | Local reconciliation complete: unified graph plane, compact project coordinate lanes, one global NOW rule, continuous cross-band explicit lineage, Objective enclosures, sparse/partial degradation, bounded attention, progressive detail, and freshness readback; current-revision browser evidence remains separate |
...
The repository contribution for the first matrix pass was locally validated, but the task-local closure was superseded by the 2026-08-22 actual-use reconciliation comment. The production renderer now shows compact spatial branch/merge paths, continuous Objective enclosures, and sparse-project collapse; synthetic wide/narrow QA is green. Fresh real-browser evidence is still a separate acceptance layer, so no Todoist source mutation or production deployment is implied by this mapping. Objective and attention projections are presentation-only and do not infer authority.
