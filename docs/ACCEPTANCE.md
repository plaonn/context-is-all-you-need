...
| Provider/controller path | negative actual-use evidence | After the issuer-derived routing revision, the user's real dogfood reached DCR, consent, and the extension callback, then showed `Todoist token exchange could not reach the authorization server; check your connection and try again.` No authenticated first GET was observed; exact documented endpoint routing is the current remediation. |
