...
The accepted Project context v1 semantics are preserved in the browser-owned core beneath a Context recovery matrix: one local Context maps to one Todoist section, recognized roots become compact horizontal project coordinate lanes, and every lane shares one semantic Y-axis with a single NOW rule running through current nodes. Each project is one continuous graph across bounded past, NOW, and near-future depth; visible paths derive only from explicit Context Predecessors, and registered short-term Objectives enclose their member nodes across that continuous graph without becoming lifecycle objects. Sparse or idle projects collapse to a compact marker, while material attention stays beside the affected node and full detail remains behind expansion. Workstream metadata remains an optional annotation rather than a mandatory row axis. Existing one-section configuration migrates locally without another OAuth registration. Project history and recent completed nodes load only when a project lane is expanded; compact reads, per-Context/project caches, bounded concurrency, freshness, and isolated partial failures keep the first board read bounded. When the existing bounded exception/decision fields are present, material blocked or decision attention is compactly surfaced while passive watching stays lower-salience; expansion reveals bounded resume-critical detail without inferring authority. See [docs/SPEC.md](docs/SPEC.md), [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md), and [docs/decisions/0005-shared-now-project-column-graph.md](docs/decisions/0005-shared-now-project-column-graph.md).
