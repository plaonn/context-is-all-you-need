...
The acceptance ledger separates local checks from live provider/browser evidence. The current ledger records wrapper-assisted success through DCR → consent → PKCE token exchange → authenticated first GET and UI render, while keeping standalone parity/cutover `not-ready` until the same flow succeeds without the temporary DevTools `window.fetch` wrapper.
